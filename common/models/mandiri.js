...
module.exports = function (Mandiri) {
  ...
  async function scraper(USER, PWD) {
    var results = {
      result: {},
      errors: [],
    };
    ...
    const accountNumbers = await page.evaluate(() => {
      ...
      const accountContainer =
        iframe.contentDocument.querySelectorAll(".acc-group")[0];
      ...
      try {
        const tempAccounts = await page.evaluate(() => {
          ...
          accountList.forEach((account) => {
            ...
            const decimalElement = account.querySelector(
              "p.nominal sup.decimal"
            );
      ...
      for (var i = 0; i < results.result.accounts.length; i++) {
        ...
        try {
          await page.evaluate((accNum) => {
            var iframe = parent.document.querySelector(
            ...
            if (transactionUIAvailable) {
              if (j !== 0) {
                ...
                await page.evaluate(
                  (first, last) => {
                    var iframe = parent.document.querySelector("frame");
                    iframe.contentDocument.querySelector("#fromDate").value =
                      first;
                    iframe.contentDocument.querySelector("#toDate").value =
                      last;
              ...
              await page.waitForFunction(
                () => {
                  ...
                  if (
                    iframe.contentDocument.querySelector(".dataTables_empty")
                  ) {
              ...
              let tempTransactions = await page.evaluate(() => {
                ...
                trs.forEach((tr) => {
                  var trx = {
                    date:
                      tr.querySelector("td.trxdate").innerText +
                      "/" +
                      yearActive,
